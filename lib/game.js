...
function(Globals, Character, Obstacle, Interaction) {
    ...
    function pan() {
        var map = $('#map');
        map.offset({
            left: Math.floor(Math.min(0, ($('body').width() - $('#editor').width()) / 2 - player.offset().x + cellSize / 2) + $('#editor').width()),
            top: Math.floor(Math.min(0, $('body').height() / 2 - player.offset().y + cellSize / 2))
        });
    }
