...
function(Globals, Character, Obstacle, Interaction) {
    ...
    function pan() {
        ...
        sceneElement.offset({
            left: Math.min(0, sceneElement.width() / 2 - player.x * cellSize + cellSize / 2),
            top: Math.min(0, sceneElement.height() / 2 - player.y * cellSize + cellSize / 2)
        });
    }
    ...
    function createGame() {
        ...
        scene = sjs.Scene({w: width, h: height, parent: sceneElement[0]});
